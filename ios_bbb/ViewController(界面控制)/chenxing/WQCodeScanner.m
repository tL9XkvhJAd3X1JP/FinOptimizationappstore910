...
- (id)init {
    self = [super init];
    if (self) {
        self.scanType = WQCodeScannerTypeBarcode;
    }
    return self;
}
- (id)initWidth:(WQCodeScannerType) wqcodeScannerType {
    self = [super init];
    if (self) {
        self.scanType = wqcodeScannerType;
    }
    return self;
}
...
//        self.timer=[NSTimer scheduledTimerWithTimeInterval:1.5 target:self selector:@selector(moveUpAndDownLine) userInfo:nil repeats: YES];
    }
}
