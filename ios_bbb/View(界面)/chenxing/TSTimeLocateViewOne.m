...
@implementation TSTimeLocateViewOne
...
{
    TSTimeLocateViewOne *_self = [[[NSBundle mainBundle] loadNibNamed:@"TSTimeLocateViewOne" owner:self options:nil] objectAtIndex:0];
    if (_self) {
...
//        [labelOnComponent release];
    ...
    switch (component) {
        ...
        case 1:
            if(row + 1 < 10)
            {
                labelOnComponent.text =  [NSString stringWithFormat:@"0%ld月",(long)(row+1)];
            }
            else
            {
        ...
        case 2:
            if(row + 1 < 10)
            {
                labelOnComponent.text =  [NSString stringWithFormat:@"0%ld日",(long)(row+1)];
            }
            else
            {
