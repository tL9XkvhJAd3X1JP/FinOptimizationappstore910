...
export default class TestListView extends Component<Props> { static navigationOptions = { title: 'FlatListDemo',
};
    constructor(props) { super(props);
        this.state = { isLoading: false,//初始化的状态，此时加载状态为不加载
        dataArray: CITY_NAMES//初始数据
        } }
    loadData=(refreshing)=>{//根据传入数据判断是上拉还是下拉
            if (refreshing) { this.setState({ isLoading: true//设置state为正在加载
                                            });
            } setTimeout(() => { let dataArray = [];
                         if (refreshing) { //如果是下拉，把城市名反转
                         for (let i = this.state.dataArray.length - 1; i >= 0; i--) { dataArray.push(this.state.dataArray[i]);
                         } } else { //如果上拉，添加数据
                         dataArray = this.state.dataArray.concat("我是底部新加的");
                         } this.setState({ dataArray: dataArray,//把数据重置为最新
    ...
    genIndicator= ()=>{ //底部加载(一个圆圈)
        ...
        <ActivityIndicator
        style={styles.indicator} size={'large'} color={'red'} animating={true} />
        <Text>正在加载更多</Text>
        </View>
    };
    ...
    render() { return ( <View style={styles.container}>
                       <FlatList
                       //1:数据的获取和渲染
                       data={this.state.dataArray} renderItem={(data) => this._renderItem(data)}//将List中的renderRow中的内容抽出来单独放成一个组件来渲染
                       ...
                       title={'Loading'} colors={['red']}//android的刷新图标颜色
