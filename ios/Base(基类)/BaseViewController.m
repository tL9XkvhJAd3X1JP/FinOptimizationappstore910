...
//  if (myUrl == nil)
//  {
//    myUrl = [[RCTBundleURLProvider sharedSettings] jsBundleURLForBundleRoot:@"index" fallbackResource:nil];
//  }
//  NSString *moduleName = @"index";
//  if ([NomalUtil isValueableString:[viewDic objectForKey:@"moduleName"]])
//  {
//    moduleName = [viewDic objectForKey:@"moduleName"];
//  }
...
RCT_EXPORT_METHOD(pushToViewWithModuleName2:(NSString *)moduleName viewControllerName:(NSString *)viewControllerName object:(NSString*)obj)
{
  //  NSString *str = [NSString stringWithFormat:@"=========%@",[[NSThread currentThread] name]];
...
RCT_EXPORT_METHOD(pushToViewWithModuleName3:(NSString *)moduleName viewControllerName:(NSString *)viewControllerName object:(NSString*)obj animate:(NSString *)animate)
{
  //  NSString *str = [NSString stringWithFormat:@"=========%@",[[NSThread currentThread] name]];
  ...
  [GCDUtil asyncThreadAndMainQueueWithBlock:^{
    BOOL isAnimate = YES;
    if ([@"false" isEqualToString:animate])
    {
      isAnimate = NO;
    }
...
-(void)pushToViewWithModuleName4:(NSString *)moduleName viewControllerName:(NSString *)viewControllerName object:(NSString*)obj animate:(BOOL)animate
{
  if ([NomalUtil isValueableString:obj])
  {
    [self.viewDic setObjectFilterNull:obj forKey:@"object"];
  }
  else
  {
    [self.viewDic removeObjectForKey:@"object"];
  }
...
-(void)pushToViewWithReactNativeModuleName:(NSString *)moduleName viewControllerName:(NSString *)viewControllerName animate:(BOOL)animate
{
  ...
  if ([NomalUtil isValueableString:moduleName])
  {
    
    RCTBridge *bridge = kAppDelegate.bridge;
    if (bridge == nil)
    {
//      RCTBridge *bridge = [[RCTBridge alloc] initWithDelegate:self launchOptions:nil];
    ...
    //Test.jsbundle
//    NSURL *jsCodeLocation = [[RCTBundleURLProvider sharedSettings] jsBundleURLForBundleRoot:@"Test" fallbackResource:nil];
...
//    if (( VCCount > 1 || self.navigationController.presentingViewController != nil))
    ...
    if (!_notRightScrollPop && self.navigationController != nil && self.navigationController.viewControllers.count > 1&& ![@"Main" isEqualToString:self.moduleName])
    {
...
-(void) pushToViewWithClassName:(NSString *) className data:(NSMutableDictionary *)dic nav:(UINavigationController *)nav dataType:(int)dataType animate:(BOOL)animate rootView:(UIView*)rootView
{
  ...
  superViewController.moduleName = [dic objectForKey:@"moduleName"];
